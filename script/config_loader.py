...
class ConfigVersion(Base):
    ...
    content: Mapped[dict] = mapped_column(type_=dict, nullable=False)
...
async def init_test_table(db_url: str) -> None:
    """
    初始化测试表结构和测试数据。
    ...
    async with engine.begin() as conn:
        ...
        # 清空测试数据
        await conn.execute(text("DELETE FROM config_versions"))
        ...
        test_data = [
            ...
            # letter_of_credit 的 profile
            {
                ...
                "content": {
                    "fields": {
                        "lc_number": {"extractor": "regex", "pattern": "LC\\d+"},
                        "amount": {"extractor": "field", "key": "amount"},
                        "issue_date": {"extractor": "field", "key": "date"},
                    },
                    "pages": ["all"],
                },
                "is_active": True,
        ...
        for data in test_data:
            await conn.execute(
                ...
                (version_tag, doc_type, config_type, content, is_active)
                VALUES (:version_tag, :doc_type, :config_type, :content, :is_active)
            """),
                data,
            )
...
if __name__ == "__main__":
    ...
    async def test():
        """测试入口：连接测试数据库，加载配置并打印结果。"""
        parser = argparse.ArgumentParser(description="从数据库加载配置")
        parser.add_argument(
            "--doc-type", default="letter_of_credit", help="单据类型"
        )
        parser.add_argument("--version", default=None, help="配置版本标签")
        parser.add_argument(
            "--init", action="store_true", help="初始化测试表和数据"
        )
        ...
        try:
            if args.init:
                await init_test_table(db_url)
                print("测试表初始化完成")

            config = await load_config(db_url, args.doc_type, args.version)

            print(f"\n配置加载成功: doc_type={args.doc_type}")
            print("-" * 50)
            print("Fields schema keys:", list(config["fields_schema"].keys()))
            print(
                "Profile fields:", list(config["profile"].get("fields", {}).keys())
            )
            print("Agg rules:", config["agg_rules"])
